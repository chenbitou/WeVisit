...
class MeetService extends BaseProjectService {
	...
	async statJoinCnt(meetId, timeMark) {
		...
		let ret = await JoinModel.groupSum(whereJoin, 'JOIN_STATUS', 'JOIN_CNT');
	...
	async join(userId, meetId, timeMark, formsList) {
		...
		data.JOIN_FORMS_LIST = formsList;
		data.JOIN_CNT = formsList.length;
		...
		let _name = (arr) => {
			...
			for (let k = 0; k < arr.length; k++) {
				for (let j = 0; j < arr[k].length; j++) {
					if (arr[k][j].mark == 'name') ret.push(arr[k][j].val);
				}
			}
			return ret;
		}
		data.JOIN_NAME_LIST = _name(formsList);
	...
	async checkMeetLimitSet(userId, meet, timeMark, nowCnt) {
		...
		let cnt = await JoinModel.sum(where, 'JOIN_CNT');

		let maxCnt = meet.MEET_MAX_JOIN_CNT;
		this._meetLog(meet, `预约次数规则,mode=本时段可预约${maxCnt}人`, `当前已预约=${cnt}人`);
		...
		if (maxCnt > 1) {
			if (cnt >= maxCnt) {
				this.AppError(`您本时段已经预约${cnt}人，每时段最多可预约${maxCnt}人，不能继续预约`);
			}
		}


		if ((cnt + nowCnt) > maxCnt) {
			this.AppError(`您本时段已经预约${cnt}人，最多还可以预约${maxCnt - cnt}人，请修改预约信息`);
		}
	}
	...
	async detailForJoin(userId, meetId, timeMark) {
		...
		let fields = 'MEET_MAX_JOIN_CNT,MEET_DAYS_SET,MEET_JOIN_FORMS, MEET_TITLE';
	...
	async getMeetList({
		...
		let fields = 'MEET_TITLE,MEET_OBJ,MEET_DAYS';
	...
	async getMyJoinList(userId, {
		...
		let fields = 'JOIN_NAME_LIST,JOIN_CNT,JOIN_COMPLETE_END_TIME,JOIN_IS_CHECKIN,JOIN_REASON,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_STATUS,JOIN_ADD_TIME';
		...
		if (util.isDefined(search) && search) {
			...
			switch (sortType) {
				...
				case 'use': { //可用
					where.JOIN_COMPLETE_END_TIME = ['>=', timeUtil.time('Y-M-D')];
					break;
				}
				case 'timeout': { //已过期
					where.JOIN_COMPLETE_END_TIME = ['<', timeUtil.time('Y-M-D')];
