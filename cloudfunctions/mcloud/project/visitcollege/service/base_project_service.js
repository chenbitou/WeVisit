...
class BaseProjectService extends BaseService {
	...
	async initSetup() {
		...
		let F = (c) => 'bx_' + c;
		const INSTALL_CL = 'setup_visitcollege';
		const COLLECTIONS = ['setup', 'admin', 'log', 'day', 'join', 'meet', 'news', 'fav', 'user', 'temp'];
		const CONST_PIC = '/images/cover.gif';

		const NEWS_CATE = '1=公告通知,2=校园导览';
		const MEET_TYPE = '1=预约';
